...
def filter_samples(samples, polygons):
    centers = np.array([(p.centroid.x, p.centroid.y) for p, h in polygons])
    tree = KDTree(centers, metric='euclidean')
    pts = []
    for s in samples:
        in_collision = False
        dist, idxs = tree.query(np.array([s[0], s[1]]).reshape(1, -1), k=1)
        idxs = idxs[0]
        if len(idxs) > 0:
            for ind in idxs:
                p, h = polygons[int(ind)]
                pt = Point(s[0], s[1])
                if p.contains(pt):
                    if h >= s[2]:
                        in_collision = True
